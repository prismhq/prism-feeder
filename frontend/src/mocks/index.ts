...
import {
  mockFeeds,
  mockFeedsResponse,
  mockScraperDiscoveryResponse,
  mockScraperPreviewResponse,
} from "./feeds";
import {
  mockEntries,
  mockEntriesResponse,
...
// WebSocket mock
export const createWebSocketMock = () => new MockWebSocket();
...
export const getMockEntriesByStatus = (
...
export const getMockCategoryStats = (categoryId: string) => {
  ...
  return {
    ...
    unread_count: categoryEntries.filter((entry) => entry.status === "unread")
      .length,
    starred_count: categoryEntries.filter((entry) => entry.status === "starred")
      .length,
  };
};
...
// Search simulation
export const searchMockEntries = (
  query: string,
  ...
  if (filters.category_id) {
    const categoryFeeds = getMockFeedsByCategory(filters.category_id as string);
